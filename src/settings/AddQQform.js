    ...
    <>
      <Dialog sx={{
        ...
        <div className="d-flex justify-content-between" onClick={onClose}>
          <DialogTitle>{editMode ? 'Edit QQForm Question' : 'Add QQForm Question'}</DialogTitle>
            ...
            <div class="col-1 mb-3 align-items-start">
              <div class="">
                ...
                <textarea name="question" type="" class="form-control" id="exampleFormControlInput1" placeholder="" onChange={handleChange}
                  value={formData.question} rows="3" cols="35"></textarea>
                  {errors.question && <span className="invalid">{errors.question}</span>}
              </div>
            </div>
          </div>
      ...
      {openPopUp && (
        <PopUp message={message} closePopup={fetchQQList} />
      )}
    </>
  );
};
